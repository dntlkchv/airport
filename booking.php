...
$flight_ids = is_array($_REQUEST['flight_id']) ? $_REQUEST['flight_id'] : [$_REQUEST['flight_id']];
...
if (isset($_POST['submit']) && $_POST['submit'] == "Забронировать") {
    ...
    foreach ($flight_ids as $flight_id) {
        foreach ([...$_GET['selected_clients'], ...array_keys($new_clients)] as $client_id) {
            $connect->query("INSERT INTO `ticket`(`ticket_date`,  `id_flight`) VALUES (NOW(),{$flight_id})");
            $connect->query("INSERT INTO `reserve`(`id_client`, `id_ticket`) VALUES ({$client_id}," . mysqli_insert_id($connect) . ")");
        }
    }

    mysqli_query($connect, "DELETE FROM cart WHERE id_user={$user['id']} AND id_flight IN (" . implode(',', $flight_ids) . ")");

    header("Location: cabinet.php");
    exit();
}

$flights = $connect->query("SELECT *, flight.id AS flight_id, port.name_air AS port_from, p1.name_air AS port_toflight, city.city_name AS city_namefrom, city1.city_name AS city_nameto FROM flight RIGHT JOIN port ON flight.from_flight=port.id RIGHT JOIN port AS p1 ON flight.to_flight=p1.id RIGHT JOIN city ON port.id_city=city.id RIGHT JOIN city AS city1 ON p1.id_city=city1.id where flight.id in (" . implode(',', is_array($_REQUEST['flight_id']) ? $_REQUEST['flight_id'] : [$_REQUEST['flight_id']]) . ")")->fetch_all(MYSQLI_ASSOC);
$сlients = [];
